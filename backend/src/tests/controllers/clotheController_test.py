...
class TestClotheController:
  ...
  def test_post_clothe(self, test_client):
      with test_client.application.app_context():
          ...
          # Formar los datos de la solicitud POST, incluyendo archivos
          data = {
              'name': 'test',
              'description': 'test',
              'id_category': '1',
              ...
              'colors[0][id_color]': '1',
              'colors[0][stock]': '10',
              'colors[0][images]': fake_image  # Incluir la imagen en los datos
          }
      
          # Enviar la solicitud POST con datos y archivos
          response = test_client.post('/api/clothe', data=data, content_type='multipart/form-data')
